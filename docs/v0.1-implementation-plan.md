...
**Progress:** Phases 0 and 1 complete. The approved product baseline is the
[PANDA v0.1 Frozen Scope Contract](v0.1-scope-contract.md), and the canonical
contract completion record is the [Phase 1 Plan](plans/phase-1.md).
...
interface ExecutionStore {
  createExecution(execution: PandaExecution): void;
  getExecution(id: string): PandaExecution | undefined;
  listExecutions(): PandaExecution[];
  updateExecution(execution: PandaExecution): void;
  appendTrace(record: TraceRecord): void;
  getTrace(executionId: string): TraceRecord[];
}
```
